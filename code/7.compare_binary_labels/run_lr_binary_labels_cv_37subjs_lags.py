...
## load data
for lag in [320]: # lags in ms
    print(lag)
    kfolds = 5
    nfolds = 5
    alphas = [0.1, 1, 10, 1e+3, 1e+4, 3e+4, 5e+4, 1e+5, 1e+7]
    out_dir = './results/encoding/ridge_accCV5_alphaCV5_concepts_129_binary/audio_boxcar_regressed_brain_hfb_shift'+str(lag)+'ms/'
    ...
    ttr = np.roll(ttr, -lag/40, axis=0) # negative shift for brain as targets
